...
type WorkerLog = {
  id: string; workerId: string; action: string; details: string
  ipAddress: string; timestamp: string
}
type Page<T> = { content: T[]; totalPages: number; totalElements: number }

export default function LogsWorkersPage() {
  const [workerIdFilter, setWorkerIdFilter] = useState('')
  const [inputValue, setInputValue] = useState('')
  const [page, setPage] = useState(0)
  const PAGE_SIZE = 20

  const { data, isLoading } = useQuery<Page<WorkerLog>>({
    queryKey: ['logs-workers', workerIdFilter, page],
    queryFn: () =>
      logService.workerLogs({ workerId: workerIdFilter || undefined, page, size: PAGE_SIZE })
        .then(r => r.data),
  })
  ...
  const handleSearch = () => {
    setPage(0)
    setWorkerIdFilter(inputValue.trim())
  }

  return (
    <div className="p-8">
      <header className="mb-8">
        <h2 className="text-[28px] font-bold text-[#f0ffe4]">Logs_Trabajadores</h2>
        <p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-[#baccaf]">
          Auditoría de actividad del panel interno — <span className="text-[#dae6d0]">{totalElements}</span> registros
        </p>
      </header>
      ...
      <div className="mb-6 flex gap-2 max-w-md">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Filtrar por Worker ID..."
        ...
        {workerIdFilter && (
          <button
            onClick={() => { setInputValue(''); setWorkerIdFilter(''); setPage(0) }}
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-[#141e10]">
                {['Timestamp', 'Trabajador ID', 'Acción', 'Detalles', 'IP'].map(h => (
